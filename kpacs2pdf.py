...
def main():
    ...
    )  # Apagar warning de img2pdf
    # start = time.time()

    # listar archivos en directorio
    # carpeta = Path(r"C:\Users\mferreyra\Desktop\Kpacks Quilmes")
    carpeta = Path(r"C:\Users\usuario\Documents\Trabajo\kpacs 28.06-30.06")
    ...
    if os.path.exists("kpacs2pdf_lista_procesados"):
        with open("kpacs2pdf_lista_procesados", "rb") as leer:
    ...
    for item in tqdm(listado, desc="Procesando imagenes", colour="green", leave=True, position=0):
        ...
        plt.savefig("kpacs2pdf_temp.temp", format="png", orientation="portrait", bbox_inches="tight")
        ...
        imagen = Image.open("kpacs2pdf_temp.temp")
        ...
        imagen.save(fp="kpacs2pdf_temp.temp", format="png")

        # if os.path.exists("kpacs2pdf_temp.temp"):
        #    os.system('attrib +h kpacs2pdf_temp.temp') #Puede generar error de permiso de acceso
        ...
        while os.path.exists(
            f"./pdf/{im.PatientID} {nombre}/{nombre.upper().replace(' ', '- ').replace(',', '- ')}- - CR- from {im.StudyDate[6:]}-{im.StudyDate[4:6]}-{im.StudyDate[0:4]} S{num} I0.pdf"
        ):
            num += 1
            if num > 20:
                break
        ...
        with open(f"./pdf/{im.PatientID} {nombre}/{archivo_nombre}.pdf", "wb") as file:
            file.write(img2pdf.convert("kpacs2pdf_temp.temp", layout_fun=layout_fun))  # type: ignore

    # borrar archivo png temporal
    if os.path.exists("kpacs2pdf_temp.temp"):
        os.remove("kpacs2pdf_temp.temp")
...
# pyinstaller --onefile -F --hiddenimport=pydicom.encoders.gdcm --hiddenimport=pydicom.encoders.pylibjpeg --upx-dir=C:\Users\usuario\source\kpacs2pdf\upx-4.0.2-win64 --icon=C:\Users\usuario\source\kpacs2pdf\splash.ico --clean kpacs2pdf.py
