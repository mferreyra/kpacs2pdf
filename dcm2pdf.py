...
for item in tqdm(listado, desc="Procesando imagenes", colour="green", leave=True, position=0):
    ...
    sup_izq = (20, 15)
    sup_der_1 = (width-160, 15)
    sup_der_2 = (width-90, 15)
    inf_izq = (20, height-70)
    inf_der = (width-90, height-70)
    
    text_sup_izq = f"{im.PatientName}\n{im.PatientSex}\n{im.PatientID}"
    text_sup_der_1 = f"{im.InstitutionName}\n"
    text_sup_der_2 = f"\n Ref: {im.ReferringPhysicianName} / Perf: ''\nStudy date: {im.StudyDate}\nStudy time: {im.StudyTime}" #TODO modificar formato de valores 
    ...
    draw.text(sup_der_1, text_sup_der_1, font=font)
    draw.text(sup_der_2, text_sup_der_2, font=font)
...
#modificar valores time a escribir
